...
def splitDocument_byTOC(window, inputDocumentTobeSplit, inputTOCfile, outputDir, openOutputFiles):
    ...
    for i in range(len(headings)):
        ...
        count += 1
        sectionContent = (
            extractSection(fileContent, headings[i], headings[i + 1])
            if i < len(headings) - 1
    ...
    if j > 0:
        mb.showwarning(
            title="Output split files",
            message=str(count)
            + " headings from the TOC file were processed and exported to the directory "
            + newoutputDir
            + ".\n\n"
            + str(j)
            + " headings were not found in the main document '"
            + inputDocumentTobeSplit
        ...
        mb.showwarning(
            title="Error found",
            message=str(count)
            + " headings from the TOC file were all successfully processed and all sections exported to the directory "
            + newoutputDir,
        )
        outputHeadingsNotfound = ""
...
def extractSection(fileContent, documentHeading, nextheading):
    ...
    if documentHeading is not None and nextheading is not None:
        ...
        pattern = re.compile(
            r"("
            + re.escape(documentHeading)
            + r"[\s*\W*]*"
            + r"\n"
            + r".+)"
            + r"(?="
            + re.escape(nextheading)
            + r"[\s*\W*]*"
            + r"\n)",
            flags=re.DOTALL | re.IGNORECASE,
        )
