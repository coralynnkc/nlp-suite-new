...
def run(
    ...
    if (
        not utf8_var
        and not ASCII_var
        and not language_detect_var.get()
        and not spelling_var.get()
        and not NLTK_unusual_var.get()
        and not lower_case_words_after_end_of_sentence_var.get()
        and not word_length_var.get()
        and not sentence_length_var.get()
    ):
        mb.showwarning(
    ...
    outputDir = IO_files_util.make_output_subdirectory(
        inputFilename, inputDir, outputDir, label="data_quality", silent=True
    )
    if outputDir == "":
        return
    ...
    if utf8_var:
        file_checker_util.check_utf8_compliance(
            GUI_util.window, inputFilename, inputDir, outputDir, openOutputFiles, True
        )
    ...
    if language_detect_var.get():
        output = file_spell_checker_util.language_detection(
            window,
            inputFilename,
            inputDir,
            outputDir,
    ...
    if lower_case_words_after_end_of_sentence_var.get():
        # @@@ case_sensitive
        output = statistics_txt_util.process_words(
            GUI_util.window,
            config_filename,
            inputFilename,
            inputDir,
            ...
            chartPackage,
            dataTransformation,
            processType="Lower case words after end-of-sentence punctuation",
            language="English",
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    y_multiplier_integer,
    spelling_auto_correct_checkbox,
    False,
    False,
    True,
    False,
...
NLTK_unusual_checkbox = tk.Checkbutton(
    window, text="NLTK unusual words", variable=NLTK_unusual_var, onvalue=1, offvalue=0
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
lower_case_words_after_end_of_sentencecheckbox = tk.Checkbutton(
    window,
    text="Lower-case words after end-of-sentence punctuation (e.g., . ? !)",
    variable=lower_case_words_after_end_of_sentence_var,
    onvalue=1,
    offvalue=0,
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    GUI_IO_util.labels_x_coordinate,
    y_multiplier_integer,
    lower_case_words_after_end_of_sentencecheckbox,
    False,
    False,
    True,
    False,
...
sentence_length_checkbox = tk.Checkbutton(
    window, text="Sentence length", variable=sentence_length_var, onvalue=1, offvalue=0
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick checkbox to run various algorithms of language detection: LANGDETECT, LANGID, spaCY, Stanza.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick checkbox to run spell-checking algorithms: Pyspellchecker, textblob.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to run the NLTK unusual word algorithm. Not all unusual words are misspelled words but the list of NLTK unusual words can be used to find misspelled words.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to obtain a list of lower-case words that occur after an end-of-sentence marker. Such words can be taken as an indication, for instance, of an incorrect conversion to text of a pdf file. Parsers (e.g., Stanford CoreNLP, spaCy) will incorrectly parse sentences with end-of-sentence markers in the wrong position (e.g., 'Robert went to. Europe for vacation').",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to run the algorithm that computes word length. Unusually long words may be an indication that words have been run together (e.g., Abraham Lincoln, AbrahamLincoln).",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
