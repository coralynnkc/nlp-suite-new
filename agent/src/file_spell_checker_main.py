...
if not IO_libraries_util.install_all_Python_packages(
    GUI_util.window,
    "spell-checker_main.py",
    ["os", "re", "nltk", "pandas", "collections", "subprocess", "time", "tkinter"],
):
    sys.exit(0)
...
def run(
    ...
    if spelling_checker_var:
        ...
        if checker_value_var == "*" or "detector" in checker_value_var:
            file_spell_checker_util.language_detection(
                window,
                inputFilename,
                inputDir,
                outputDir,
        ...
        if (
            checker_value_var == "*"
            or "autocorrect" in checker_value_var
            or "pyspellchecker" in checker_value_var
            or "textblob" in checker_value_var
        ):
            autocorrect_df, pyspellchecker_df, textblob_df = file_spell_checker_util.spellcheck(
                inputFilename, inputDir, checker_value_var, check_withinSubDir_spell_checker_var
            )
        if checker_value_var == "*" or "autocorrect" in checker_value_var:
            autocorrect_file_name = IO_files_util.generate_output_file_name(
                inputFilename, inputDir, outputDir, ".csv", "spell_autocorrect"
            )
        ...
        if checker_value_var == "*" or "BERT" in checker_value_var:
            ...
            BERT_output = BERT_util.spell_checker_bert(
                window, inputFilename, inputDir, outputDir, "", chartPackage, dataTransformation
            )
        ...
        if checker_value_var == "*" or "textblob" in checker_value_var:
            textblob_file_name = IO_files_util.generate_output_file_name(
                inputFilename, inputDir, outputDir, ".csv", "spell_textblob"
            )
        ...
        if "Replace" in checker_value_var:
            file_spell_checker_util.spelling_checker_cleaner(
                window, inputFilename, inputDir, outputDir, openOutputFiles, config_filename
            )
    ...
    # spell checking by CoreNLP algorithms -------------------------------------------------------------------------------------------------
        ...
        if check_withinSubDir and (not spelling_checker_var) and (not bydictionary_value_var):
            ...
            #
            outputFiles = file_spell_checker_util.check_for_typo_sub_dir(
                inputDir,
                outputDir,
                selectedCsvFile_var,
                openOutputFiles,
            ...
            outputFiles = file_spell_checker_util.check_for_typo(
                inputDir,
                outputDir,
                selectedCsvFile_var,
                openOutputFiles,
...
def build_NER_list():
    ...
    if (
        NER_value_var.get() != "*"
        and NER_value_var.get() != ""
        and selected_NER_list_var.get() != ""
        and NER_value_var.get() not in selected_NER_list_var.get()
    ):
...
Levenshtein_distance_checkbox = tk.Checkbutton(
    window,
    text="Run Levensthein' distance algorithm",
    state="normal",
    variable=Levenshtein_distance_var,
    onvalue=1,
    offvalue=0,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_coordinate,
    "Check the spelling of words in your corpus using the Levenshtein edit distance.\nSelect any of the options listed below.",
)

by_all_words_var.set(0)
by_all_words_checkbox = tk.Checkbutton(
    window,
    text="Check all words against one another",
    state="normal",
    variable=by_all_words_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_indented_coordinate,
    "Check each word in your corpus against each and every other word in the corpus.",
)

bydictionary_value_var.set(0)
bydictionary_value_checkbox = tk.Checkbutton(
    window,
    state="disabled",
    text='Check all words against dictionary "true" values',
    variable=bydictionary_value_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_indented_coordinate,
    'Check all words against a set of "true" spelling values in a csv file.\nOnly the first column will be considered; any other colum will be ignored.',
)

dictionary_button = tk.Button(
    window,
    width=20,
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, dictionary_button, True
)
...
openInputFile_button = tk.Button(
    ...
    text="",
    command=lambda: IO_files_util.openFile(window, selectedCsvFile_var.get()),
)
# the button widget has hover-over effects (no_hover_over_widget=False) and the info displayed is in text_info
# the two x-coordinate and x-coordinate_hover_over must have the same values
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    GUI_IO_util.file_search_byWord_openInputFile_button_pos,
    y_multiplier_integer,
    ...
    90,
    GUI_IO_util.file_search_byWord_openInputFile_button_pos,
    "Open selected csv dictionary file",
)

selectedCsvFile = tk.Entry(
    window, width=GUI_IO_util.file_search_byWord_widget_width, state="disabled", textvariable=selectedCsvFile_var
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_search_byWord_selectedCsvFile_pos, y_multiplier_integer, selectedCsvFile
)

byNER_value_var.set(0)

byNER_value_checkbox = tk.Checkbutton(
    window,
    state="normal",
    text="Check words by their NER tag",
    variable=byNER_value_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, byNER_value_checkbox
)

NER_value_lb = tk.Label(window, text="Select NER tag for computing word similarity")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate + 20, y_multiplier_integer, NER_value_lb, True
)
NER_value = tk.OptionMenu(
    window, NER_value_var, "*", "CITY", "COUNTRY", "STATE_OR_PROVINCE", "LOCATION", "ORGANIZATION", "PERSON"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate + 50, y_multiplier_integer, NER_value, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 600, y_multiplier_integer, selected_NER_list
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, similarity_value_lb, True
)
similarity_value = tk.Entry(window, width=5, textvariable=similarity_value_var)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, similarity_value, True
)

check_withinSubDir_var.set(0)
check_withinSubDir_checkbox = tk.Checkbutton(
    window, text="Check WITHIN each subdirectory only", variable=check_withinSubDir_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 600, y_multiplier_integer, check_withinSubDir_checkbox
)

spelling_checker_checkbox = tk.Checkbutton(
    window,
    text="Run spelling checker",
    state="normal",
    variable=spelling_checker_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_coordinate,
    "Check the spelling of words in your corpus using various spelling algorithms edit distance.",
)

checker_value_var.set("*")
spelling_checker_value_lb = tk.Label(window, text="Select script")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate + 150, y_multiplier_integer, spelling_checker_value_lb, True
)
spelling_checker_value = tk.OptionMenu(
    window,
    checker_value_var,
    "*",
    "Language detector",
...
check_withinDir_spell_checker_checkbox = tk.Checkbutton(
    window, text="Check WITHIN main directory only", variable=check_withinDir_spell_checker_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate + 600, y_multiplier_integer, check_withinDir_spell_checker_checkbox
)
...
def activate_all_options(*args):
    ...
    if Levenshtein_distance_var.get():
        ...
        if by_all_words_var.get():
            bydictionary_value_checkbox.configure(state="disabled")
            byNER_value_checkbox.configure(state="disabled")
        if bydictionary_value_var.get():
            by_all_words_checkbox.configure(state="disabled")
            byNER_value_checkbox.configure(state="disabled")
        if byNER_value_var.get():
            by_all_words_checkbox.configure(state="disabled")
            NER_value.config(state="normal")
            selected_NER_list.config(state="normal")
    else:
        spelling_checker_checkbox.configure(state="normal")
    if spelling_checker_var.get():
...
TIPS_lookup = {
    "Word similarity (Levenshtein edit distance)": "TIPS_NLP_Word similarity (Levenshtein edit distance).pdf",
...
TIPS_options = (
    ...
    "utf-8 text encoding",
    "Spelling checker",
    "Word similarity (Levenshtein edit distance)",
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to find the edit distance of any word in your input document(s), regardless of their NER tag."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox to search input txt file(s) using the values contained in a csv dictionary file.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to find the edit distance of words in your input document(s) by their selected NER values."
        + GUI_IO_util.msg_Esc,
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
