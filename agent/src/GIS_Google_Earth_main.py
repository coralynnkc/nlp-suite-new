...
def run(
    ...
    if locationColumnName == "":
        print(
            "No location column selected, No csv column containing location names has been selected.\n\nPlease, select a column and try again."
        )
        return
    ...
    if withHeader:
        IO_csv_util.get_columnNumber_from_headerValue(headers, locationColumnName, inputFilename)
    ...
    if group_var == 1 and group_number_var == 1:
        print(
            "Only One Group Chose for Multiple Group Mapping The group box is ticked for multiple groups mapping but only one group is chose.\n\nPlease, check your input and try again."
        )
        return
    ...
    if group_var == 1 and len(icon_csv_field_var_name) < 1:
        print(
            "No CSV field for Group Split Criterion Found The group box is ticked for multiple groups mapping but no csv field is specified for group splitting criterion.\n\nPlease, check your input and try again. "
        )
        return
    ...
    outputDir = IO_files_util.make_output_subdirectory(inputFilename, inputDir, outputDir, label="GIS-GEP", silent=True)
    if outputDir == "":
        return
    if heat_map_var:
    ...
    filesToOpen = GIS_pipeline_util.GIS_pipeline(
        config_filename,
        inputFilename,
        inputDir,
        outputDir,
