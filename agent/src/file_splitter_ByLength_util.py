...
def splitAt(text,index):
    ...
    while True:
        try:
            ...
            IO_user_interface_util.timed_alert(4000, 'Warning',
...
def split_byLength(input_path,filename,output_path, maxLength, inSentence=False):
    ...
    if maxLength > len(word_tokenize_stanza(stanzaPipeLine(text))):
        IO_user_interface_util.timed_alert( 2000, 'File split warning', 'The length of file ' + filename + ' is less than ' + str(maxLength))
