...
def computePairwiseDistances(
    window,
    inputFilename,
    outputDir,
    headers,
    locationColumnNumber,
    locationColumnNumber2,
    locationColumnName,
    locationColumnName2,
    distinctValues,
    geolocator,
    ...
    startTime = IO_user_interface_util.timed_alert(
        window, 2000, "Analysis start", "Started running GIS distance at", True, "", True, "", True
    )
    if distinctValues:
        distanceoutputFilename = IO_files_util.generate_output_file_name(
            inputFilename,
            "",
            outputDir,
            ".csv",
    ...
    # latin-1
    with (
        open(inputFilename, newline="", encoding=encodingValue, errors="ignore") as inputFile,
        open(distanceoutputFilename, "w", newline="", encoding=encodingValue, errors="ignore") as outputFile,
    ):
        ...
        for index, row in dt.iterrows():
            ...
            if (
                str(currentLocation1) != ""
                and str(currentLocation1) != "nan"
                and str(currentLocation2) != ""
                and str(currentLocation2) != "nan"
            ):  # nan Not A Numeric value SHOULD NOT BE NECESSARY!!!
                # nan Not A Numeric value
                if (
                    str(row[locationColumnNumber + 1]) == ""
                    or str(row[locationColumnNumber + 2]) == ""
                    or str(row[locationColumnNumber + 1]) == "nan"
                    or str(row[locationColumnNumber + 2]) == "nan"
                ):
                    print(currRecord, "     WAYPOINTS NOT NUMERIC (nan) ", currentLocation1)
                    waypoints1 = ""
                    waypoints2 = ""
                elif (
                    str(row[locationColumnNumber + 4]) == ""
                    or str(row[locationColumnNumber + 5]) == ""
                    or str(row[locationColumnNumber + 4]) == "nan"
                    or str(row[locationColumnNumber + 5]) == "nan"
                ):
                    ...
                    try:
                        ...
                        print(
                            ...
                            + " ("
                            + headers[locationColumnNumber + 2]
                            + ") of your input csv file does not contain proper Latitude values for the first location.\n\nPlease, check your selected 'Column containing location names' and/or input csv filename and try again. "
                        )
                        filesToOpen.append("")
                        return filesToOpen
                    try:
                    ...
                    try:
                        ...
                        print(
                            ...
                            + " ("
                            + headers[locationColumnNumber + 6]
                            + ") of your input csv file does not contain proper Longitude values for the second location.\n\nPlease, check your selected 'Select the second column containing location names (for distances)' and/or input csv filename and try again."
                        )
                        filesToOpen.append("")
                        return filesToOpen
...
def computeDistancesFromSpecificLocation(
    window,
    inputFilename,
    outputDir,
    geolocator,
    ...
    startTime = IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis start",
        "Started running GIS distance from " + baselineLocation + " at",
        True,
        "",
        True,
        "",
    ...
    if not InputIsGeocoded:
        ...
        if not IO_internet_util.check_internet_availability_warning("GIS geocoder"):
            return
        startTime = IO_user_interface_util.timed_alert(
            window, 2000, "Analysis start", "Started running GIS geocoder at", True, "", True, "", True
        )
        ...
        locationsNotFoundFilename = IO_files_util.generate_output_file_name(
            inputFilename, "", outputDir, ".csv", "GIS", geoName, "Not-Found", locationColumnName, "", False, True
        )
        ...
        if inputIsCoNLL:
            ...
            locations = GIS_location_util.extract_NER_locations(
                inputFilename, filenamePositionInCoNLLTable, encodingValue, split_locations, datePresent
            )
        else:
            locations = GIS_location_util.extract_csvFile_locations(
                inputFilename, withHeader, locationColumnNumber, encodingValue
            )

        if locations is None or len(locations) == 0:
            filesToOpen.append("")
            return filesToOpen
        geocodedLocationsoutputFilename, locationsNotFoundFilename = GIS_geocode_util.geocode(
            window, locations, inputFilename, outputDir, locationColumnName, geocoder, "", encodingValue
        )
        ...
        try:
            ...
            print(
                ...
                + "\nwith non geocoded input. Most likely, the error is due to an encoding error. Your current encoding value is "
                + encodingValue
                + ".\n\nSelect a different encoding value and try again. "
            )
            filesToOpen.append("")
        ...
        if location is None:
            print(
                "Input error, The baseline location cannot be geocoded. \n\nPlease, enter a new baseline location and try again."
            )
            filesToOpen.append("")
            return filesToOpen
    ...
    with open(distanceoutputFilename, "w", newline="", encoding=encodingValue, errors="ignore") as outputFile:
        ...
        for index, row in dt.iterrows():
            ...
            if currentLocation != "" and currentLocation != "nan":  # nan Not A Numeric value SHOULD NOT BE NECESSARY!!!
                try:
                    ...
                    print(
                        ...
                        + " ("
                        + headers[locationColumnNumber + 2]
                        + ") of your input csv file does not contain proper Latitude values for your location.\n\nPlease, check your selected 'Column containing location names' and/or input csv filename and try again."
                    )
                    filesToOpen.append("")
                    return filesToOpen
                try:
            ...
            if waypoints2 != "":
                if (
                    (not distinctValues)
                    or ([waypoints1, waypoints2] not in currList)
                    and (baselineLocation != currentLocation)
                ):
                    currList.append([waypoints1, waypoints2])
                    distMiles = distance.distance(waypoints1, waypoints2).miles
                    distKm = distance.distance(waypoints1, waypoints2).km
