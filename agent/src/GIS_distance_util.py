            ...
            if (str(currentLocation1)!='' and str(currentLocation1)!='nan' and str(currentLocation2)!='' and str(currentLocation2)!='nan'): #nan Not A Numeric value SHOULD NOT BE NECESSARY!!!
                ...
                if str(row[locationColumnNumber+1])=='' or str(row[locationColumnNumber+2])=='' or str(row[locationColumnNumber+1])=='nan' or str(row[locationColumnNumber+2])=='nan':
                    ...
                    try:
                        ...
                        print("Input file error, Column number " + str(locationColumnNumber+1) + " (" + headers[locationColumnNumber+2] + ") of your input csv file does not contain proper Latitude values for the first location.\n\nPlease, check your selected 'Column containing location names' and/or input csv filename and try again. ")
                        filesToOpen.append('')
                        return filesToOpen
                    try:
                    ...
                    try:
                        ...
                        print("Input file error, Column number " + str(locationColumnNumber+4) + " (" + headers[locationColumnNumber+5] +") of your input csv file does not contain proper Latitude values for the second location.\n\nPlease, check your selected 'Select the second column containing location names (for distances)' and/or input csv filename and try again.")
                        filesToOpen.append('')
                        return filesToOpen
                    try:
                        float(row[locationColumnNumber+5])
                    except:
                        print("Input file error, Column number " + str(locationColumnNumber+5) + " (" + headers[locationColumnNumber+6] +") of your input csv file does not contain proper Longitude values for the second location.\n\nPlease, check your selected 'Select the second column containing location names (for distances)' and/or input csv filename and try again.")
                        filesToOpen.append('')
                        return filesToOpen
...
def computeDistancesFromSpecificLocation(window,inputFilename,outputDir,geolocator,geocoder,InputIsGeocoded,baselineLocation,headers,locationColumnNumber,locationColumnName,distinctValues,withHeader,inputIsCoNLL,split_locations,datePresent,filenamePositionInCoNLLTable,encodingValue):
    ...
    if InputIsGeocoded == False:
        ...
        if location is None:
            print("Input error, The baseline location cannot be geocoded. \n\nPlease, enter a new baseline location and try again.")
            filesToOpen.append('')
            return filesToOpen
    ...
    # geocoded input
        ...
        try:
            ...
            print("Input file error, There was an error in the function 'Compute GIS distance from specific location' reading the input file\n" + str(inputFilename) + "\nwith geocoded input. Most likely, the error is due to an encoding error. Your current encoding value is " + encodingValue + ".\n\nSelect a different encoding value and try again.")
            filesToOpen.append('')
            return filesToOpen
        ...
        for index, row in dt.iterrows():
            ...
            if currentLocation!='' and currentLocation!='nan': #nan Not A Numeric value SHOULD NOT BE NECESSARY!!!
                try:
                    ...
                    print("Input file error, Column number " + str(locationColumnNumber+1) + " (" + headers[locationColumnNumber+2] + ") of your input csv file does not contain proper Latitude values for your location.\n\nPlease, check your selected 'Column containing location names' and/or input csv filename and try again.")
                    filesToOpen.append('')
                    return filesToOpen
                try:
                    float(row[locationColumnNumber+2])
