...
def get_local_release_version():
    ...
    if os.path.isfile(release_version_file):
        with open(release_version_file, encoding="utf-8", errors="ignore") as file:
...
def get_GitHub_release_version(silent=False):
    ...
    if not IO_internet_util.check_internet_availability_warning(
        "GUI_util.py (Function Automatic check for NLP Suite newest release version on GitHub)"
    ):
        GitHub_newest_release = "0.0.0"
...
def check_GitHub_release(local_release_version: str, silent=False):
    ...
    if "Not Found" not in GitHub_newest_release and old_version:  # GitHub_newest_release != local_release_version:
        ...
        result = mb.askyesno(
            "NLP Suite Outdated",
            "You are running the NLP Suite release version "
            + str(local_release_version).rstrip()
            + ", an OLD version."
            + "\n\nA NEW version of the NLP Suite has been released on GitHub: "
            ...
            + "\n\nWOULD YOU LIKE TO SEE WHAT IS NEW IN THE RELEASE VERSION "
            + str(GitHub_newest_release).rstrip()
            + "?",
        )
        if result:
...
def display_release():
    ...
    if sys.platform == "darwin":  # Mac OS
        y_multiplier_integer = -0.8
    else:
        y_multiplier_integer = -0.9
    ...
    if GitHub_newest_release != "0.0.0":
        ...
        mb.showwarning(
            ...
            message="The GitHub release version is displayed on the top left-hand corner of the GUI as 0.0.0.\n\nWithout internet the newest release available on GitHub cannnot be retrieved.",
        )
    return local_release_version, GitHub_newest_release


def selectFile_set_options(
    window, IsInputFile, checkCoNLL, inputFilename, input_main_dir_path, title, fileType, extension
):
    ...
    if len(currentFilename) > 0:
        ...
        initialFolder = ""
    # get the file
    filename = IO_files_util.selectFile(
        window, IsInputFile, checkCoNLL, title, fileType, extension, None, initialFolder
    )
    if len(filename) == 0:
        return
    else:
...
def display_IO_setup(window, IO_setup_display_brief, config_filename, config_input_output_alphabetic_options, *args):
    ...
    if IO_setup_display_brief:
        if (
            "license" in config_filename
            or "package_language" in config_filename
            or "external_software" in config_filename
        ):
            return missing_IO
...
def check_fileName(scriptName, file_type, config_input_output_numeric_options):
    ...
    if inputFilename.get() == "":
        ...
        if config_input_output_numeric_options[0] == 1:
            if "CoNLL" not in inputFilename.get():
                err_msg = (
                    "The script '" + scriptName + "' requires a " + file_type + " FILE in INPUT.\n\n"
        ...
        if config_input_output_numeric_options[0] == 1 or config_input_output_numeric_options[0] == 3:
            err_msg = (
                "The script '" + scriptName + "' requires a " + file_type + " FILE in INPUT.\n\n"
...
def activateRunButton(config_filename, IO_setup_display_brief, scriptName, missing_IO, silent=False):
    ...
    run_button_state, open_setup_IO_GUI = config_util.check_missing_IO(
        window, config_filename, scriptName, IO_setup_display_brief, missing_IO, silent
    )
    if open_setup_IO_GUI:
        silent = True
        missing_IO, config_filename = setup_IO_configuration_options(
            IO_setup_display_brief, scriptName, silent, open_setup_IO_GUI=False
        )
    if missing_IO != "":
        ...
        if (
            (config_input_output_numeric_options[0] == 0 and inputFilename.get() != "")
            or (config_input_output_numeric_options[1] == 0 and input_main_dir_path.get() != "")
            or err_msg != ""
        ):
...
def set_IO_brief_values(config_filename, y_multiplier_integer):
    ...
    missing_IO = ""
    if config_filename != "":
        config_input_output_alphabetic_options, missing_IO, config_file_exists = config_util.read_config_file(
    ...
    if config_input_output_alphabetic_options[0][1] != "":  # check that there is a file path
        ...
        inputFilename.set(
            IO_files_util.open_file_removing_date_from_filename(
                window, config_input_output_alphabetic_options[0][1], False
            )
        )
        input_main_dir_path.set("")
        file_date_label = ""
    ...
    if config_input_output_alphabetic_options[1][1] != "":  # check that there is a dir path
        try:
            ...
            mb.showwarning(
                ...
                message="The config file "
                + config_filename
                + " is not an expected I/O configuration file with the following header values: I/O configuration label	Path, Sort order, Item separator character(s), Date format, Date positionan.\n\nPlease, select an expected I/O file.",
            )
            return "", "", config_input_output_alphabetic_options, missing_IO
        # date label already added in NLP_setup_IO_main
        # remove date in input_main_dir_path since it will be used in ALL GUIs
        input_main_dir_path.set(
            IO_files_util.open_directory_removing_date_from_directory(
    ...
    if (config_input_output_alphabetic_options[0][1] == "") and (config_input_output_alphabetic_options[1][1] == ""):
...
def IO_config_setup_brief(window, y_multiplier_integer, config_filename, scriptName, silent):
    IO_setup_button = tk.Button(
        window,
        width=GUI_IO_util.select_file_directory_button_width,
        text="Setup INPUT/OUTPUT configuration",
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.setup_pop_up_text_widget,
        y_multiplier_integer,
        openTextWidget_button,
        True,
        False,
        True,
        False,
        ...
        "Button currently not used. Will eventually open a pop-up text-entry widget where users can paste text to be used temporarily to run the algorithms behind the GUI, instead of either Default or any selected I/O csv config file options.",
    )
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.IO_configuration_menu + GUI_IO_util.open_file_button_brief,
        y_multiplier_integer,
    ...
    openInputDirectory_button = tk.Button(
        window,
        width=GUI_IO_util.open_file_directory_button_width,
        text="",
        command=lambda: IO_files_util.open_directory_removing_date_from_directory(
            window, input_main_dir_path.get(), True
        ),
    )
    # place widget with hover-over info
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.IO_configuration_menu + GUI_IO_util.open_inputDir_button_brief,
        y_multiplier_integer,
        openInputDirectory_button,
        True,
        False,
        True,
        False,
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.IO_configuration_menu + GUI_IO_util.open_outputDir_button_brief,
        y_multiplier_integer,
        openOutputDirectory_button,
        True,
        False,
        True,
        False,
        90,
        x_coordinate_hover_over,
        "Open OUTPUT files directory",
    )

    # Open csv config file
    openInputConfigFile_button = tk.Button(
        window,
        width=GUI_IO_util.open_file_directory_button_width,
        text="",
    ...
    y_multiplier_integer = GUI_IO_util.placeWidget(
        window,
        GUI_IO_util.IO_configuration_menu + GUI_IO_util.open_config_file_button_brief,
        y_multiplier_integer,
        openInputConfigFile_button,
        True,
        False,
        True,
        False,
...
def IO_config_setup_full(window, y_multiplier_integer):
    ...
    global \
        select_inputFilename_button, \
        select_input_main_dir_button, \
        select_input_secondary_dir_button, \
    ...
    if config_input_output_numeric_options[0] > 0:
        ...
        if config_input_output_numeric_options[0] == 1:  # single CoNLL file
            ...
            select_inputFilename_button = tk.Button(
                window,
                width=GUI_IO_util.select_file_directory_button_width,
                text="Select INPUT CoNLL table",
                command=lambda: selectFile_set_options(
                    window,
                    True,
                    True,
                    inputFilename,
                    input_main_dir_path,
                    "Select INPUT CoNLL table (csv file)",
                    [("CoNLL csv file", ".csv")],
                    ".csv",
                ),
            )
        ...
        if config_input_output_numeric_options[0] == 4:  # any type file (used in NLP.py)
            ...
            select_inputFilename_button = tk.Button(
                window,
                width=GUI_IO_util.select_file_directory_button_width,
                text="Select INPUT file (any type)",
                command=lambda: selectFile_set_options(
                    window,
                    True,
                    False,
                    inputFilename,
                    input_main_dir_path,
                    "Select INPUT file (any file type: pdf, docx, html, txt, csv, conll); switch extension type below near File name:",
                    [
                        ("txt file", "*.txt"),
                        ("csv file", "*.csv"),
                        ("pdf file", "*.pdf"),
        ...
        if config_input_output_numeric_options[0] == 5:  # txt/html
            ...
            select_inputFilename_button = tk.Button(
                window,
                width=GUI_IO_util.select_file_directory_button_width,
                text="Select INPUT file (txt, html)",
                command=lambda: selectFile_set_options(
                    window,
                    True,
                    False,
                    inputFilename,
                    input_main_dir_path,
                    "Select INPUT file (txt, html); switch extension type below near File name:",
                    [("txt file", "*.txt"), ("html file", "*.html")],
                    "*.*",
                ),
            )
        if config_input_output_numeric_options[0] == 6:  # txt/csv
            ...
            select_inputFilename_button = tk.Button(
                window,
                width=GUI_IO_util.select_file_directory_button_width,
                text="Select INPUT file (txt, csv)",
                command=lambda: selectFile_set_options(
                    window,
                    True,
                    False,
                    inputFilename,
                    input_main_dir_path,
                    "Select INPUT file (txt, csv); switch extension type below near File name:",
        ...
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, select_inputFilename_button, True
        )
    ...
    if config_input_output_numeric_options[1] == 1:  # main directory input
        ...
        select_input_main_dir_button = tk.Button(
            ...
            command=lambda: selectDirectory_set_options(
                window, input_main_dir_path, output_dir_path, "Select INPUT files directory", True
            ),
        )
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, select_input_main_dir_button, True
        )

        # setup a button to open Windows Explorer on the selected main input directory
        openDirectory_button = tk.Button(
            window,
            width=GUI_IO_util.open_file_directory_button_width,
            text="",
            command=lambda: IO_files_util.open_directory_removing_date_from_directory(
    ...
    if config_input_output_numeric_options[2] == 1:  # secondary directory input
        ...
        select_input_secondary_dir_button = tk.Button(
            window,
            width=GUI_IO_util.select_file_directory_button_width,
            text="Select INPUT secondary directory",
            command=lambda: selectDirectory_set_options(
                window, input_main_dir_path, input_secondary_dir_path, "Select INPUT secondary TXT directory"
            ),
        )
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, select_input_secondary_dir_button, True
        )
        ...
        openDirectory_button = tk.Button(
            ...
            width=GUI_IO_util.open_file_directory_button_width,
            text="",
            command=lambda: IO_files_util.openExplorer(window, input_secondary_dir_path.get()),
        )
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window,
            GUI_IO_util.IO_configuration_menu,
            ...
            90,
            GUI_IO_util.IO_configuration_menu,
            "Open INPUT files SECONDARY directory",
        )

        inputSecondaryDir_lb = tk.Label(window, textvariable=input_secondary_dir_path)
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, inputSecondaryDir_lb
        )
    ...
    if config_input_output_numeric_options[3] == 1:  # output directory
        ...
        select_output_dir_button = tk.Button(
            window,
            width=GUI_IO_util.select_file_directory_button_width,
            text="Select OUTPUT files directory",
            command=lambda: selectDirectory_set_options(
                window, input_main_dir_path, output_dir_path, "Select OUTPUT files directory"
            ),
        )
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, select_output_dir_button, True
        )
        ...
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window, GUI_IO_util.entry_box_x_coordinate, y_multiplier_integer, outputDir_lb
        )
...
def open_NLP_setup_IO_main(config_input_output_numeric_options, config_filename):
    ...
    if not error_found:
        ...
        list_of_files = glob.glob(
            GUI_IO_util.configPath + os.sep + "*.csv"
...
def setup_IO_configuration_options(IO_setup_display_brief, scriptName, silent, open_setup_IO_GUI):
    ...
    if open_setup_IO_GUI:  # open the NLP_setup_IO_main GUI to setup a new I/O configuration
        ...
        config_filename = IO_files_util.selectFile(
            window,
            True,
            False,
            "Select INPUT csv I/O config file; Press CANCEL to setup new I/O configuration options",
            [("csv file", ".csv")],
            ".csv",
            None,
    ...
    if "_ALL_" not in scriptName and "package_language" not in scriptName:
        try:
            config_input_output_numeric_options = [6, 1, 0, 1]
            config_input_output_alphabetic_options = config_util.get_template_config_csv_file(
                config_input_output_numeric_options, ""
            )
    ...
    if (
        "NLP_setup_IO_main" not in scriptName
    ):  # if the NLP_setup_IO_main is already opened, you do not want to open it again
        if not silent and missing_IO != "":
            mb.showwarning(
                title="Warning",
                message='Since required Input/Output (I/O) information is missing for the "'
                + str(setup_IO_menu_var.get())
                + '", for your convenience the NLP Suite will open the "NLP_setup_IO_main" GUI where you can enter the required I/O information.\n\nThe RUN button in the current GUI will be disabled until you enter the required I/O information.',
            )
        ...
        if open_setup_IO_GUI:
            ...
            changed_setup_IO_config(scriptName, IO_setup_display_brief, silent=False, open_setup_IO_GUI=False)
            open_setup_IO_GUI = False
            if "NLP_menu_main" not in scriptName and "package_language" not in scriptName:
...
def display_about_release_team_cite_buttons(scriptName):
    if "NLP_welcome_main" in scriptName or "NLP_menu_main" in scriptName:
        ...
        about_button = tk.Button(
            window, text="About", width=15, height=1, foreground="red", command=lambda: GUI_IO_util.about()
        )
        # place widget with hover-over info
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window,
        ...
        release_history_button = tk.Button(
            window,
        ...
        team_button = tk.Button(
            window, text="NLP Suite team", width=15, height=1, foreground="red", command=lambda: GUI_IO_util.list_team()
        )
        # place widget with hover-over info
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window,
        ...
        cite_button = tk.Button(
            window, text="How to cite", width=15, height=1, foreground="red", command=lambda: GUI_IO_util.cite_NLP()
        )
        # place widget with hover-over info
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window,
...
def GUI_top(config_input_output_numeric_options, config_filename, IO_setup_display_brief, scriptName, silent=False):
    ...
    if "NLP_menu_main" not in scriptName and config_input_output_numeric_options != [0, 0, 0, 0]:
    ...
    if os.path.isfile(os.path.join(GUI_IO_util.libPath, "LICENSE-NLP-Suite-1.0.txt")) and (
        IO_libraries_util.check_inputPythonJavaProgramFile("license_GUI.py")
    ):
    ...
    setup_IO_menu_var.trace(
        "w", lambda x, y, z: changed_setup_IO_config(scriptName, IO_setup_display_brief, open_setup_IO_GUI=True)
    )
...
def get_hover_over_info(package_display_area_value):
    ...
    if package_display_area_value != "":
        ...
        NLP_current_settings = (
            "There are no currently selected options. You will not be able to run many of the NLP algorithms."
        )
...
def display_setup_hover_over(y_multiplier_integer):
    ...
    setup_menu_lb = tk.OptionMenu(
        window,
        setup_menu,
        "Setup preferences",
        "Setup NLP package (parsers & annotators) and corpus language",
...
def watch_video(videos_lookup, scriptName):
    ...
    if videos_dropdown_field.get() != "Watch videos":
        ...
        if not request.ok:
            mb.showinfo(
                title="video error",
                message="There was an error in opening the video '"
                + videos_dropdown_field.get()
                + "' on YouTube for the GUI '"
                + scriptName
...
def changed_setup_IO_config(scriptName, IO_setup_display_brief, silent=False, open_setup_IO_GUI=False):
    ...
    if (
        "license" not in config_filename
        and "package_language" not in config_filename
        and "external_software" not in config_filename
    ):
        missing_IO = display_IO_setup(
            window, IO_setup_display_brief, config_filename, config_input_output_alphabetic_options
        )
...
def GUI_bottom(
    ...
    if (
        ...
        and "NLP_menu_main" not in scriptName
        and "NLP_setup_" not in scriptName
    ):
        ...
        open_csv_output_label = tk.Checkbutton(
            ...
            command=lambda: trace_checkbox(
                open_csv_output_label, open_csv_output_checkbox, "Open output files", "Do NOT open output files"
            ),
        )
        ...
        if scriptName == "data_visualization_1_main.py":
            charts_type_options = [
                "Excel & Python options: Bar, Bubble, Line, Pie, Radar, and Scatter charts (OPEN GUI)",
                "Boxplot (Open GUI)",
                "Comparative bar charts (Open GUI)",
                "Time mapper (Open GUI)",
                "Geographic maps (Open GUI)",
                "Wordcloud (Open GUI)",
            ]
        elif scriptName == "data_visualization_2_main.py":
            charts_type_options = [
                "_________________ Excel & Python options",
                "Bar chart",
                "Bubble chart",
                "Line chart",
                "Pie chart",
                "Radar chart",
                "Scatter plot",
                "_________________ Open GUI",
                "Colormap (Open GUI)",
                "Geographic maps (Open GUI)",
                "Gephi (Open GUI)",
                "Sankey flowchart (Open GUI)",
                "Sunburst chart (Open GUI)",
                "Treemap chart (Open GUI)",
                "Wordcloud (Open GUI)",
            ]
        else:
            charts_type_options = [
                ...
                "Pie chart",
                "_________________ Open GUI",
                "Bubble, Radar, Scatter plots (Open GUI)",
                "Boxplot (Open GUI)",
                "Colormap (Open GUI)",
                "Comparative bar charts (Open GUI)",
                "Geographic maps (Open GUI)",
        ...
        data_transformation_menu_lb = tk.OptionMenu(
            window, data_transformation_options_widget, *data_transformation_options
        )
        # place widget with hover-over info
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window,
        ...
        def run_data_transformation(*args):
            if "Normalize" in data_transformation_options_widget.get():
                mb.showwarning(
                    title="Warning",
                    message="All charts involving multiple documents are automatically normalized by document size",
                )
        ...
        def open_GUI(*args):
            if (
                "Bubble" in charts_type_options_widget.get()
                or "Comparative" in charts_type_options_widget.get()
                or "Box" in charts_type_options_widget.get()
                or "Time" in charts_type_options_widget.get()
            ):
        ...
        def run_data_tool(*args):
            if "data_manipulation_main.py" not in scriptName and "manipulation" in data_tools_options_widget.get():
                call("python data_manipulation_main.py", shell=True)
            if "statistics_csv_main.py" not in scriptName and "statistics" in data_tools_options_widget.get():
                call("python statistics_csv_main.py", shell=True)
            if (
                "data_visualization_1_main.py" not in scriptName
                and "visualization 1" in data_tools_options_widget.get()
            ):
                call("python data_visualization_1_main.py", shell=True)
            if (
                "data_visualization_2_main.py" not in scriptName
                and "visualization 2" in data_tools_options_widget.get()
            ):
                call("python data_visualization_2_main.py", shell=True)
            if "sample_corpus_main.py" not in scriptName and "sampling" in data_tools_options_widget.get():
    ...
    reminders_util.checkReminder(
        "*", reminders_util.title_options_NLP_Suite_reminders, reminders_util.message_NLP_Suite_reminders
    )
    ...
    if (
        "NLP_menu_main" not in scriptName
        and "package_language" not in config_filename
        and "external_software" not in config_filename
    ):
    ...
    # TODO RUN button
    if ("narrative_analysis" in scriptName) or ("NLP_setup_" not in scriptName and ("ALL_main" not in scriptName)):
        # place widget with hover-over info
        y_multiplier_integer = GUI_IO_util.placeWidget(
            window,
            GUI_IO_util.run_button_x_coordinate,
    ...
    run_button_state, missing_IO = activateRunButton(
        config_filename, IO_setup_display_brief, scriptName, missing_IO, silent
    )
