...
def compute_csv_column_statistics_NoGroupBy(inputFilename, outputDir, openOutputFiles,
    ...
    if len(stats)>0:
        ...
        IO_csv_util.df_to_csv(df, outputFilename, headers=None, index=False,
...
def compute_csv_column_statistics_groupBy(inputFilename, outputDir, outputFileNameLabel, groupByField: list, plotField: list, chart_title_label, chartPackage, dataTransformation):
    filesToOpen=[]
    ...
    if not set(groupByField).issubset(set(IO_csv_util.get_csvfile_headers(inputFilename))):
        print('Groupby field error', "The selected groupby fields (" + ", ".join(groupByField) + ") are not in the headers (" + ", ".join(IO_csv_util.get_csvfile_headers(inputFilename)) + ") of the file " + inputFilename)
    ...
    if len(groupByField)>0:
        try:
            ...
            IO_user_interface_util.timed_alert(5000, 'Input file error',
...
def compute_csv_column_statistics(inputFilename,outputDir, outputFileNameLabel,
    ...
    if len(groupByList)>0:
        outputFiles=compute_csv_column_statistics_groupBy(inputFilename,outputDir,outputFileNameLabel,groupByList,plotList,chart_title_label,chartPackage, dataTransformation)
        if outputFiles!=None:
            if isinstance(outputFiles, str):
...
def compute_csv_column_frequencies(inputFilename, inputDataFrame, outputDir,
    ...
    if len(plot_cols) == 0:
        print('Missing field', 'You have not selected the csv field for which to compute frequencies.\n\nPlease, select the field and try again.')
        return filesToOpen
