...
def run(
    ...
    if annotator_dictionary_var and annotator_dictionary_file_var == "":
        if annotator_dictionary_file_var == "":
            mb.showwarning(
                title="Warning",
                message="You have selected to annotate your corpus using dictionary entries, but you have not provided the required .csv dictionary file.\n\nPlease, select a .csv dictionary file and try again.",
            )
            return
    ...
    # dict annotate
        ...
        if not IO_libraries_util.check_inputPythonJavaProgramFile("html_annotator_gender_dictionary_util.py"):
            return
    ...
    # plot annotate
        ...
        if year_state_var == "" or firstName_entry_var == "":
            mb.showwarning(
                title="Warning",
                message="The plot option requires both 'By year/state' value and first name(s) in the 'Enter first name(s)' widget.\n\nPlease, enter the required information and try again.",
            )
            return
        else:
            outputFiles = html_annotator_gender_dictionary_util.SSA_annotate(
                year_state_var, firstName_entry_var, outputDir
            )
    ...
    if openOutputFiles:
        ...
        if nFile > 5:
            mb.showwarning(
                title="Warning",
                message="There are too many output files ("
                + str(nFile)
...
CoreNLP_gender_annotator_checkbox = tk.Checkbutton(
    window,
    text="Annotate nouns & pronouns gender (via CoreNLP Gender annotator - Neural Network)",
    variable=CoreNLP_gender_annotator_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, CoreNLP_gender_annotator_checkbox
)

# TODO list of male/female names https://nlp.stanford.edu/projects/gender.shtml
CoreNLP_download_gender_file_checkbox = tk.Checkbutton(
    window, text="Download CoreNLP gender file", variable=CoreNLP_download_gender_file_var, onvalue=1, offvalue=0
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer, CoreNLP_download_gender_file_checkbox, True
)

CoreNLP_upload_gender_file_checkbox = tk.Checkbutton(
    window, text="Upload CoreNLP gender file", variable=CoreNLP_upload_gender_file_var, onvalue=1, offvalue=0
)
# html_annotator_gender_select_dictionary_file_annotator
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_TIPS_x_coordinate, y_multiplier_integer, CoreNLP_upload_gender_file_checkbox
)

annotator_dictionary_var.set(0)
annotator_dictionary_checkbox = tk.Checkbutton(
    window,
    text="Annotate first names by gender (via selected dictionary file)",
    variable=annotator_dictionary_var,
    onvalue=1,
    offvalue=0,
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, annotator_dictionary_checkbox
)

annotator_dictionary_button = tk.Button(
    window,
    text="Select dictionary file ",
    command=lambda: get_dictionary_file(window, "Select INPUT dictionary file", [("dictionary files", "*.csv")]),
)
...
openInputFile_button = tk.Button(
    ...
    state="disabled",
    text="",
    command=lambda: IO_files_util.openFile(window, annotator_dictionary_file_var.get()),
)
...
annotator_dictionary_file = tk.Entry(
    window,
    width=GUI_IO_util.html_annotator_gender_annotator_dictionary_file_width,
    textvariable=annotator_dictionary_file_var,
)
annotator_dictionary_file.config(state="disabled")
# html_annotator_gender_select_dictionary_file_annotator
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_TIPS_x_coordinate, y_multiplier_integer, annotator_dictionary_file
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_coordinate,
    "Tick the checkbox to search for use of first names by state/year of birth using the US Social Security first names databases",
)

year_state_lb = tk.Label(window, text="By US state/year")
# html_annotator_gender_select_dictionary_file_annotator
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_TIPS_x_coordinate, y_multiplier_integer, year_state_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.html_annotator_gender_by_type_dropdown,
    "Use the dropdown menu to select the desired first name search",
)

firstName_entry_lb = tk.Label(window, text="Enter name(s)")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.open_setup_x_coordinate, y_multiplier_integer, firstName_entry_lb, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    "Enter the comma-separated first names whose use by state/year of birth you wish to process",
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    ...
    90,
    GUI_IO_util.labels_x_indented_coordinate,
    "Open the US Social Security website where you can download first name databases: National data and State-specific data\nYou must be connected to the internet",
)
#
# https://www.ssa.gov/oact/babynames/limits.html
new_SS_folders_var.set(0)
new_SS_folders_checkbox = tk.Checkbutton(
    window,
    text="Generate new US Social Security files (by US State, Year of birth, US State & Year of birth)",
    variable=new_SS_folders_var,
    onvalue=1,
    offvalue=0,
    command=lambda: get_new_SS_folders(window),
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
    ...
    90,
    GUI_IO_util.open_TIPS_x_coordinate,
    "After downloading the US SS Nation data and State-specific data, tick the checkbox to prepare the data for NLP Suite algorithms",
)
...
def get_new_SS_folders(window):
    ...
    mb.showwarning(
        title="Warning",
        message="You are about to select the two folders where you downloaded and unzipped the most up-to-date gender names databases 'National data' and 'State-specific data' from the US Social Security website\n\nhttps://www.ssa.gov/oact/babynames/limits.html\n\nYOU WILL LOOP TWICE TO SELECT BOTH FOLDERS.",
    )
    for _i in range(2):
        if len(new_SS_folders) == 0:
            ...
            title = (
                "Select INPUT new SS folder for the 'State-specific data' database (downloaded folder 'namesbystate')"
            )
        ...
        if len(folderPath) > 0:
            if folderPath in new_SS_folders:
                mb.showwarning(
                    title="Warning",
                    message="You have selected the same SS folder twice. The two folders are expected to be different.\n\nPlease, try again.",
                )
                return
    ...
    if len(new_SS_folders) != 2:
        mb.showwarning(
            title="Warning",
            message="You were expected to select two folders (downloaded names & namesbystate).\n\nPlease, try again if you wish to update your SS gender names databases.",
        )
        new_SS_folder_var.set("")
        new_SS_folders.clear()
    ...
    if os.path.isfile(GUI_IO_util.namesGender_libPath + os.sep + "SS_yearOfBirth.csv") and os.path.isfile(
        GUI_IO_util.namesGender_libPath + os.sep + "SS_state_yearOfBirth.csv"
    ):
        mb.showwarning(
            title="Warning",
            message="The new Social Security files by year of birth and state were successfully created in the directory "
            + GUI_IO_util.namesGender_libPath,
        )
    else:
        mb.showwarning(
            title="Warning",
            message="The new Social Security files by year of birth and state files failed to be created.\n\nPlease, check your input 'names' and 'namesbystate' directories and your output directory "
            + GUI_IO_util.namesGender_libPath
            + " and try again.",
        )
...
TIPS_lookup = {
    ...
    "CoreNLP Coref": "TIPS_NLP_Stanford CoreNLP coreference resolution.pdf",
}
TIPS_options = (
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    if not IO_setup_display_brief:
        y_multiplier_integer = GUI_IO_util.place_help_button(
            window, help_button_x_coordinate, y_multiplier_integer, "NLP Suite Help", GUI_IO_util.msg_txtFile
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(
        ...
        y_multiplier_integer,
        "NLP Suite Help",
        "Please, tick the checkbox if you wish to run the Stanford CoreNLP gender annotator. The CoreNLP gender annotator is based on CoreNLP annotator which, unfortunately, only has about 60\\% accuracy. The algorithm annotates the gender of both first names and personal pronouns (he, him, his, she, her, hers).\n\nThe CoreNLP annotator uses a neural network approach. This annotator requires a great deal of memory.",
    )
    y_multiplier_integer = GUI_IO_util.place_help_button(
        window,
        help_button_x_coordinate,
