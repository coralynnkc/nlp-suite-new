...
if not IO_libraries_util.install_all_Python_packages(
    GUI_util.window, "html_annotator_gender_dictionary_util", ["tkinter", "pandas", "stanza"]
):
    sys.exit(0)
...
def dictionary_annotate(
    ...
    if len(tempOutputFiles) == 0:
        ...
        with open(NER_fileName, encoding="utf-8", errors="ignore") as infile:
            ...
            headers = next(reader)
        [i for i, item in enumerate(headers) if item]
...
def build_dictionary_yob(source_file_path):
    result = []
    for folder, _subs, files in os.walk(source_file_path):
        for filename in files:
