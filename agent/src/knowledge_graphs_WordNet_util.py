...
def disaggregate_GoingDOWN(WordNetDir, outputDir, wordNet_keyword_list, noun_verb):
    ...
    IO_user_interface_util.timed_alert(
        2000, "Analysis end", "Finished running WordNet (Zoom IN/DOWN) at", True, "", True, startTime
    )
    return filesToOpen
...
def aggregate_GoingUP(
    ...
    IO_user_interface_util.timed_alert(
        2000, "Analysis end", "Finished running WordNet (Zoom OUT/UP) at", True, "", True, startTime, True
    )
...
def Wordnet_bySentenceID(
    ...
    filesToOpen = []
    startTime = IO_user_interface_util.timed_alert(
        2000, "Analysis start", "Started running WordNet charts by sentence index at", True, "", True, "", False
    )
    ...
    df = pd.DataFrame(
        Row_list, index=["Form", "Lemma", "POS", "WordNet Category", "Sentence ID", "Document ID", "Document"]
    )
    ...
    if chartPackage != "No charts":
        ...
        if len(outputFiles) > 0:
            filesToOpen.extend(outputFiles)
    IO_user_interface_util.timed_alert(
        2000, "Analysis end", "Finished running WordNet charts by sentence index at", True, "", True, startTime
    )
