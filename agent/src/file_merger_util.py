...
def file_merger(
    ...
    openOutputFiles,
    processSubdir=None,
    saveFilenameInOutput=None,
    merge_separator=False,
    startString="<@#",
    endString="#@>",
    embedSubdir=None,
    separator="__",
    writeRootDirectory=True,
    configFileName="",
):
    docNum = 0
    ...
    if processSubdir is None:  # cancel
        return
    if processSubdir:
        tmpList = IO_files_util.getFileList_SubDir(
        ...
        if fileError > 0:
            ...
            if fileLengthError > 0:
                msg = (
                    msg
                    + str(fileLengthError)
                    + " files have combined path + filename greater than Windows limit of 260 characters.\n\n"
                )
            msg = (
                msg
                + "Files with errors have been listed in command line.\n\nPlease, check these files carefully and try again."
            )
        else:
