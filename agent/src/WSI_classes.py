...
class Clusterer:
    ...
    def group_wordpiece(self, embeddings, word, do_wordpiece):
        ...
        ongoing_word = []
        ongoing_rep = []
        for _i, tup in enumerate(embeddings):
            if not do_wordpiece:
                if tup[0] == word:
                    data.append(tup[1])
            else:
        ...
        for i, k in enumerate(ks):
            try:
                ...
                if "should be >=" in str(e):
                    print(
                        f':-( The frequency of "{w}" ({freq}) in your dataset is less than the number of sense clusters ({k}) to be produced.\n\n'
                        "Please try to either lower the range of sense clusters to be produced or choose more frequent words to analyze and try again.\n\n"
