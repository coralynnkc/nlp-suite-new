...
def run_kg_wordnet(inputFilename, inputDir, outputDir,openOutputFiles,
        chartPackage,
        dataTransformation,
        csv_file,
        aggregate_POS_var,
    ...
    if aggregate_POS_var==False and disaggregate_var==False and annotate_file_var== False and aggregate_lemmatized_var==False and aggregate_bySentenceID_var==False and extract_nouns_verbs_from_CoNLL_var==False and extract_proper_nouns==False and extract_improper_nouns==False:
        print("Missing required information, No options have been selected.\n\nPlease, tick one of the available options and try again.")
        return False

    if disaggregate_var==True and len(wordNet_keyword_list)==0:
        print("Missing required information, You have selected to run the option 'Zoom IN/DOWN to find related words', but you have not entered any keywords required to run the script.\n\nPlease, enter the keywords and try again.")
        return False
    ...
    if annotate_file_var:
        if IO_libraries_util.check_inputPythonJavaProgramFile('html_annotator_dictionary_util.py') == False: #Check if the scrit is here 
            ...
            if csv_file != '':
                ...
                print("Option not available, You have selected to run the option 'Annotate corpus' but you have not selected a csv input file.\n\nPlease, select an INPUT csv file and try again.")
                return
            sel_col = ''
            ...
            if 'Term' == headers[0]:
                ...
                print("Wrong input file, You have selected to run the option 'Annotate corpus' but the selected input csv file is NOT a file generated by the Zoom IN/DOWN algorithm. Only that type of csv file is admitted; that file should have the header 'Term'.\n\nPlease, select the expected csv file and try again.")
                return
    ...
    if extract_proper_nouns==1 or extract_improper_nouns==1:
        ...
        if csv_file!='':
            ...
            print("Option not avaialble, You have selected to run the option 'Extract PROPER/IMPROPER nouns' but you have not selected a csv input file.\n\nPlease, select an INPUT csv file and try again.")
            return
        sel_col=''
        # Word is the term used by other scripts; but the WordNet script uses Term; just in case we change that...
        if 'Term' == headers[0]:
            ...
            print("Wrong input file, You have selected to run the option 'Extract PROPER/IMPROPER nouns' but the selected input csv file is NOT a file generated by the Zoom IN/DOWN algorithm. Only that type of csv file is admitted; that file should have the header 'Term'.\n\nPlease, select the expected csv file and try again.")
            return
        if noun_verb!='NOUN':
            print("Option not available, You have selected to run the option 'Extract PROPER/IMPROPER nouns' with VERB.\n\nPlease, select NOUN and try again.")
            return
        if len(csv_file)==0:
            print("Missing input file, You have selected to run the option 'Extract PROPER/IMPROPER nouns'. The algorithm expects in input a WordNet dictionary file previously exported with the 'Zoom IN/DOWN' algorithm for nouns.\n\nPlease, select an input WordNet dictionary file and try again.")
            return
        check_column=0
    ...
    if aggregate_lemmatized_var==True:
        ...
        if len(csv_file)==0:
            print("Missing required information, You have selected to run the option 'Zoom OUT/UP to find higher-level aggregates', but you have not selected the Input csv file for " + noun_verb + " required to run the script.\n\nPlease, select the Input file and try again.")
            return False
        if noun_verb=='NOUN' and 'nouns_lemma' not in csv_file:
            if hidden_noun_lemma_csv != '':
                csv_file_var = hidden_noun_lemma_csv
            else:
                print("Missing required information, You have selected to run the option 'Zoom OUT/UP to find higher-level aggregates' with the 'NOUN' option but the csv file currently selected does not contain the expected subscript 'nouns_lemma'.\n\nIf this an overshigth, click on the Select INPUT CSV file button to select a different csv file and try again.")
        ...
        if noun_verb=='VERB' and 'verbs_lemma' not in csv_file:
            if hidden_verb_lemma_csv !='':
                csv_file_var = hidden_verb_lemma_csv
            else:
                print("Missing required information, You have selected to run the option 'Zoom OUT/UP to find higher-level aggregates' with the 'VERB' option but the csv file currently selected does not contain the expected subscript 'verbs_lemma'.\n\nIf this an overshigth, click on the Select INPUT CSV file button to select a different csv file and try again.")
