...
def run(
    ...
    if compute_pairwise_distances and not inputIsGeocoded:
        print(
            "Warning, You are running the GIS algorithm for pairwise distances with an input file that seems to be non-geocoded. You cannot run this option with non-geocoded data. You can only run the option 'Compute distances from baseline location.'\n\nPlease, select a different option or a geocoded file with six columns and try again."
        )
        return

    if compute_pairwise_distances and (locationColumn == ""):
        print(
            "Warning, You are running the GIS algorithm for pairwise distances. You must select the column containing the First location names.\n\nPlease, using the dropdown menu, select the column of the FIRST location names and try again. "
        )
        return

    if compute_pairwise_distances and inputIsGeocoded and len(headers) < 6:
        print(
            "Warning, You are running the GIS algorithm for pairwise distances. But your input file has fewer than the expected 6 headers (Location1, Latitude1, Longitude1, Location2, Latitude2, Longitude2):\n\n' "
            + str(headers)
            + "\n\nPlease, select a different file and try again."
        )
        return

    if not compute_pairwise_distances and inputIsGeocoded and (locationColumn == "" or locationColumn2 == ""):
        print(
            "Warning, You are running the GIS distance algorithm using two sets of locations. You must select the columns containing the First and Second location names.\n\nPlease, using the dropdown menu, select the column of the FIRST and SECOND location names and try again. "
        )
        return

    if compute_baseline_distances and baselineLocation != "" and locationColumn == "":
        print(
            "Warning, You are running the GIS distance algorithm from the baseline location '"
            + baselineLocation
            + "'. You must select the column containing the First location name.\n\nPlease, using the dropdown menu, select the column of the FIRST location names and try again."
        )
        return
    ...
    if withHeader:
        ...
        if len(locationColumn2) > 0:
            locationColumnNumber2 = IO_csv_util.get_columnNumber_from_headerValue(
                headers, locationColumn2, inputFilename
            )
    ...
    if compute_baseline_distances and baselineLocation != "":
        filesToOpen = GIS_distance_util.computeDistancesFromSpecificLocation(
            inputFilename,
            outputDir,
            geolocator,
            geocoder,
    ...
    if compute_pairwise_distances:
        filesToOpen = GIS_distance_util.computePairwiseDistances(
            inputFilename,
            outputDir,
            headers,
            locationColumnNumber,
