...
def check_for_typo(
    inputDir, outputDir, openOutputFiles, chartPackage, dataTransformation, NERs, similarity_value, by_all_tokens_var
):
    filesToOpen = []
    ...
    CoreNLPdir, software_url, missing_external_software, errorFound = IO_libraries_util.get_external_software_dir(
        "spell_checker_main", "Stanford CoreNLP"
    )
    ...
    if len(df_simple) > 0 and len(df_complete) > 0:
        ...
        IO_user_interface_util.timed_alert(
            GUI_util.window, 3000, "Word similarity end", "Finished running Word similarity at", True
        )
...
def spelling_checker_cleaner(window, inputFilename, inputDir, outputDir, openOutputFiles):
    ...
    for i in range(len(original)):
        if (
            (isinstance(corrected[i], str) and corrected[i] != "") or (not math.isnan(corrected[i]))
        ):  # the correction is neither empyt string, nor NaN, then both original and corrected will be added to the input lists
            input_original.append(original[i])
            input_corrected.append(corrected[i])
    file_cleaner_util.find_replace_string(
        window, inputFilename, inputDir, outputDir, openOutputFiles, input_original, input_corrected, False
    )
