        ...
        for doc in this_intrude:
            ...
            if repeat_star:
                print(
                    id
                    + ","
                    + str(len(keywords.keys()))
                    + ","
                    + doc
                    + ","
                    + IO_csv_util.dressFilenameForCSVHyperlink(dir_path)
                    + ","
                    + IO_csv_util.dressFilenameForCSVHyperlink(filePath)
                    + ","
                    + str(similar[doc])
                    + ",,**************************************"
                )
...
def main(
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        "", inputDir, outputDir, ".csv", "SSR", "INTRD", "", "", "", False, True
    )
    ...
    sys.stdout = f
    print(
        "Group identifier (Folder name),Documents in group,Intruder document,Group path,Document path,Similarity index ( <"
        + str(similarityIndex_base)
        + ")"
    )
    ...
    for dir in dirs:
        ...
        print(
            ...
            + str(len(dirs))
            + "; Folder name: "
            + dir.split(os.path.sep)[-2]
        )
        sys.stdout = f
        intruder_list, id_list, has_intruder, freq_intruder, num_doc = check(
            dir,
            actors,
            nlp,
            intruder_list,
    ...
    ##
    outputFilename = IO_files_util.generate_output_file_name(
        "", inputDir, outputDir, ".csv", "SSR", "Intrud", "freq", "", "", False, True
    )
    filesToOpen.append(outputFilename)
    ...
    if len(intruder_list) <= 320:
        print(
            "Similarity index cutoff,Number of intruders,Percentage of intruders among all documents for all groups (folders),Number of intruded groups (folders),Percentage of intruded groups (folders),List of intruded groups (folders),List of intruded documents"
        )
        print(
            str(similarityIndex_base)
            + ","
            + str(freq_intruder)
            + ","
            + str(round((freq_intruder / num_doc) * 100, 2))
            + ","
            + str(freq_intruded_folder)
            + ","
            + str(round((freq_intruded_folder / num_folder) * 100, 2))
            + ","
            + "; ".join(d.strip() for d in id_list)
            + ","
            + "; ".join(d.strip() for d in intruder_list)
        )
    elif len(intruder_list) <= 640:
        print(
            "Similarity index cutoff,Number of intruders,Percentage of intruders among all documents for all groups (folders),Number of intruded groups (folders),Percentage of intruded groups (folders),List of intruded groups (folders),List of intruded documents,Continued list of intruded documents"
        )
        print(
            str(similarityIndex_base)
            + ","
        ...
        print(
            ...
            + "; ".join(d.strip() for d in intruder_list[:320])
            + ","
            + "; ".join(c for c in intruder_list[320:640])
            + ","
            + "; ".join(c for c in intruder_list[640:])
        )
    f_e.close()
    sys.stdout = terminal_output
