...
if not IO_libraries_util.install_all_Python_packages(GUI_util.window, "file_splitter_ByNumber_util", ["os"]):
    sys.exit(0)
...
def run(inputFilename, outputPath, post_number_string=""):
    ...
    with open(inputFilename, encoding="utf-8", errors="ignore") as iptf:  # read each line
        ...
        while line:
            ...
            line = iptf.readline()
    subfilename = (
        outputPath + "/" + title + "_prenumber" + ".txt"
    ...
    for line in lines:
        ...
        if len(spaceless) > 0 and spaceless[0].isdigit():  # not a blank line and the head character is a number
            ...
            if (
                len(post_number_string) > 0 and spaceless[i : i + j] != post_number_string
            ):  # if the number has post_number_string behind it
                recognized = False
        else:
            recognized = False
