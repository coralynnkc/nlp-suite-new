...
if not IO_libraries_util.install_all_Python_packages(
    GUI_util.window, "utf8_compliance_util", ["os", "re", "tkinter", "chardet"]
):
    sys.exit(0)
...
def predict_encoding(window, inputFilename, inputDir, outputDir, n_lines=20):
    ...
    mb.showwarning(
        title="Predicted encoding",
        message=encoding
        + "\n\nis the predicted encoding, using first "
        + str(n_lines)
        + " lines, of the file\n\n"
        + inputFilename,
    )
...
def check_utf8_compliance(
    ...
    if len(inputDocs) == 0:
        mb.showwarning(
            title="Input error",
            message="There are no files of type txt in the selected input directory to be checked for utf-8 compliance.\n\nPlease, select a different directory (or file) and try again.",
        )
        return
    ...
    IO_user_interface_util.timed_alert(
        ...
        2000,
        "Analysis end",
        "Finished running utf-8 compliance test at",
        True,
        "",
        True,
        startTime,
...
def check_empty_file(inputFilename, inputDir, configFileName):
    ...
    if nDocs == 1:
        ...
        if emptyFiles == 0:
            ...
            if emptyFiles == 1:
                msg = (
                    "1 file is empty in the directory\n\n"
                    + inputDir
                ...
                msg = (
                    str(emptyFiles)
                    + "files are empty in the directory\n\n"
                    + inputDir
                    + '\n\nPlease, check the command line/prompt searching for the word "empty."'
                )
    mb.showerror(title="File empty", message=msg)
    return
