        ...
        if not csvExist:
            writer.writerow(
                [
                    ...
                    "SEARCH_WORD",
                    "SENTENCE",
                    "Sentence ID of FIRST_OCCURRENCE",
                    "RELATIVE_POSITION",
        ...
        for sent in sentences_:
            ...
            for token in tokens_:
                ...
                if t.lower() == kwtokens[kwindex] or (
                    lemmatization and (t.lower() in kwlist[kwindex] or kwtokens[kwindex] == wordnet.morphy(t))
                    ...
                    kw = False
                    kwindex = 0
            if kw:  # if keyword is detected, generate the next subfile
                frequency += 1
                presubfile = subfilePath
                if frequency == 1:
                    first_occurrence_index = sentence_index
                if not first_occurrence or frequency <= 1:
                    subfileindex += 1
                    subfilePath = outputPath + os.sep + title + "_" + str(subfileindex) + ".txt"
                    if first_occurrence and subfileindex == 1:
                        subfilePath = outputPathone + os.sep + title + "_" + str(subfileindex) + ".txt"
                    if first_occurrence and subfileindex == 2:
