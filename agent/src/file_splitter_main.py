...
if not IO_libraries_util.install_all_Python_packages(GUI_util.window, "file_splitter_main.py", ["os", "tkinter"]):
    sys.exit(0)
...
def run(
    ...
    IO_user_interface_util.timed_alert(
        GUI_util.window, 2000, "Analysis start", "Started running " + menu_option + " at", True
    )
    ...
    if extract_sentences_var:
        ...
        sentence_analysis_util.extract_sentences(
            window, inputFilename, inputDir, outputDir, extract_sentences_search_words_var
        )
        return
    # split by Beginning Middle and End K sentences
        ...
        filesToOpen = file_splitter_ByBME_K_sentences_util.sample_doc_beginning_middle_end(
            window,
            config_filename,
            inputFilename,
            inputDir,
            ...
            if splitByTOC:
                if TOC_filename == "":
                    mb.showwarning(
                        ...
                        + menu_option
                        + " - requires a Table of Content file in input.\n\nPlease, select the file and try again.",
                    )
                    return
                import file_splitter_ByTOC_util

                file_splitter_ByTOC_util.splitDocument_byTOC(
                    GUI_util.window, file, TOC_filename, outputDir, openOutputFiles
                )
            # split <@# #@> --------------------------------------------------------------------------------------
                ...
                subDir, nFiles = file_splitter_merged_txt_util.run(
                    file, split_mergedFile_separator_entry_begin, split_mergedFile_separator_entry_end, outputDir
                )
                mb.showwarning(
                    title="Exported files",
                    message=str(nFiles)
                    + " split files were created in the subdirectory of the output directory\n\n"
                    + subDir,
                )
                return
            # split file length --------------------------------------------------------------------------------------
                ...
                if splitByFileLength and split_docLength == "":
                    mb.showwarning(
                        ...
                        message="The selected option - "
                        + menu_option
                        + " - requires a valid number of words for splitting the document.\n\nPlease, enter a value and try again.",
                    )
                    return
                import file_splitter_ByLength_util

                file_splitter_ByLength_util.split_byLength(
                    GUI_util.window, inputDir, file, specialOutputPath, maxLength=int(split_docLength), inSentence=True
                )
            # split keyword --------------------------------------------------------------------------------------
            elif splitByKeyword:
                ...
                if file[-4:] == ".txt":
                    ...
                    file_splitter_ByKeyword_txt_util.run(
                        file, specialOutputPath, keyword_value_var, first_occurrence_var, lemmatize_var
                    )
                elif file[-4:] == ".csv":
                    import file_splitter_ByKeyword_conll_util

                    file_splitter_ByKeyword_conll_util.run(
                        file, specialOutputPath, keyword_value_var, first_occurrence_var, generateForm=False
                    )
                ...
                if file[-4:] != ".csv":
                    mb.showwarning(
                        title="Warning",
                        message="The 'split by Document ID' function expects in input a csv file with at least one field labeled 'Document ID'.\n\nPlerase, select a csv file and try again.",
                    )
                    return
                else:
...
split_mergedFile_separator_entry_begin = tk.Entry(
    window, width=GUI_IO_util.widget_width_extra_short, textvariable=split_mergedFile_separator_entry_begin_var
)
split_mergedFile_separator_entry_end = tk.Entry(
    window, width=GUI_IO_util.widget_width_extra_short, textvariable=split_mergedFile_separator_entry_end_var
)

split_mergedFile_var.set(0)
split_mergedFile_checkbox = tk.Checkbutton(
    window,
    text="Split a merged file with filename embedded in separator strings",
    state="normal",
    variable=split_mergedFile_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, split_mergedFile_checkbox, True
)
...
docLength_var.set(0)
docLength_checkbox = tk.Checkbutton(
    window,
    text="Split by number of words",
    variable=docLength_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
...
current_docLength = tk.Entry(
    window, width=GUI_IO_util.widget_width_extra_short, state="disabled", textvariable=current_docLength_var
)
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(
    window,
...
extract_BME_K_sentences_checkbox = tk.Checkbutton(
    window,
    text="Split by Beginning-Middle-End K-sentences (extract sentences)",
    variable=extract_BME_K_sentences_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, extract_BME_K_sentences_checkbox, True
)
...
def getTOCFile():
    if TOC_var.get():
        filePath = tk.filedialog.askopenfilename(
            title="Select INPUT txt file", initialdir=GUI_util.inputFilename.get(), filetypes=[("txt files", "*.txt")]
        )
        if len(filePath) > 0:
...
TOC_checkbox = tk.Checkbutton(
    window,
    text="Split using Table of Contents (TOC)",
    variable=TOC_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, TOC_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, TOC_filename
)

keyword_var.set(0)
keyword_checkbox = tk.Checkbutton(
    window, text="Split by word(s)", variable=keyword_var, onvalue=1, offvalue=0, command=lambda: activate_all_options()
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, keyword_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_splitter_lemmatize_pos, y_multiplier_integer, lemmatize_checkbox, True
)

first_occurrence_var.set(0)
first_occurrence_checkbox = tk.Checkbutton(
    window, text="First occurrence", variable=first_occurrence_var, onvalue=1, offvalue=0
)
first_occurrence_checkbox.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_splitter_first_occurrence_pos, y_multiplier_integer, first_occurrence_checkbox
)

extract_sentences_var.set(0)
extract_sentences_checkbox = tk.Checkbutton(
    window,
    text="Split by word(s) (extract sentences)",
    variable=extract_sentences_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, extract_sentences_checkbox, True
)
...
string_var.set(0)
string_checkbox = tk.Checkbutton(
    window, text="Split by string", variable=string_var, onvalue=1, offvalue=0, command=lambda: getDocLength()
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, string_checkbox, True
)
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.IO_configuration_menu, y_multiplier_integer, string_value
)

blankLine_var.set(0)
blankLine_checkbox = tk.Checkbutton(
    window,
    text="Split by an empty blank line",
    variable=blankLine_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, blankLine_checkbox
)

number_var.set(0)
number_checkbox = tk.Checkbutton(
    window,
    text="Split by a line that starts with a number (like a bullet point)",
    variable=number_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, number_checkbox, True
)
# post_num is the string behind each number
# (if the numbers in the txt are in the form of "1. ", "2. ", then the post_num should be ". ")

post_num_string_value_lb = tk.Label(
    window, text="Enter characters to be expected after a bullet-point number (e.g., . )"
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_splitter_post_num_string_value_lb_pos, y_multiplier_integer, post_num_string_value_lb, True
)

post_num_string_value_var.set("")
post_num_string_value = tk.Entry(
    window, width=GUI_IO_util.widget_width_extra_short, textvariable=post_num_string_value_var
)
post_num_string_value.configure(state="disabled")
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.file_splitter_post_num_string_value_pos, y_multiplier_integer, post_num_string_value
)

split_csv_by_documentID_var.set(0)
split_csv_by_documentID_checkbox = tk.Checkbutton(
    window,
    text="Split csv merged file into separate files by Document ID",
    variable=split_csv_by_documentID_var,
    onvalue=1,
    offvalue=0,
    command=lambda: activate_all_options(),
)
y_multiplier_integer = GUI_IO_util.placeWidget(
    window, GUI_IO_util.labels_x_coordinate, y_multiplier_integer, split_csv_by_documentID_checkbox
)
...
def activate_all_options(*args):
    ...
    if extract_BME_K_sentences_var.get():
...
TIPS_lookup = {
    ...
    "File merger": "TIPS_NLP_File merger.pdf",
    "File splitter": "TIPS_NLP_File splitter.pdf",
}
TIPS_options = (
    "File splitter",
    "File merger",
    "File handling in NLP Suite",
    "File manager",
    "Filename checker",
    "Filename matcher",
    "File classifier (By date)",
    "File classifier (By NER)",
    "File content checker & converter",
    "Text encoding (utf-8)",
    "Spelling checker",
)
...
def help_buttons(window, help_button_x_coordinate, y_multiplier_integer):
    if not IO_setup_display_brief:
        y_multiplier_integer = GUI_IO_util.place_help_button(
            window, help_button_x_coordinate, y_multiplier_integer, "NLP Suite Help", GUI_IO_util.msg_anyFile
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
            window, help_button_x_coordinate, y_multiplier_integer, "NLP Suite Help", GUI_IO_util.msg_anyData
        )
        y_multiplier_integer = GUI_IO_util.place_help_button(
...
message = (
    "Some of the algorithms behind this GUI rely on a specific NLP package to carry out basic NLP functions (e.g., sentence splitting, tokenizing, lemmatizing) for a specific language your corpus is written in.\n\nYour selected corpus language is "
    + str(language)
