...
def run(inputFilename, folium_pinmap_outputFilename, folium_heatmap_outputFilename):
    ...
    locationsData["coordinates"] = list(zip(locationsData.Latitude, locationsData.Longitude, strict=False))
    # locationsData

    # making coordinates into list
    locationsData["coordinates"].tolist()
    ...
    for i in GISLocation.index:
        if pd.notnull(GISLocation["Latitude"][i]):
            folium.CircleMarker(
                [GISLocation["Latitude"][i], GISLocation["Longitude"][i]],
                radius=3,
                color="#CD3181",
                fill_color="#CD3181",
            ).add_to(mappingLocation)
            folium.Marker(
                location=[GISLocation["Latitude"][i], GISLocation["Longitude"][i]],
                tooltip="Click me!",
    ...
    heatmap = folium.Map(location=[51.5074, 0.1278], zoom_start=3)
