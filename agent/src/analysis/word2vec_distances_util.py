...
def compute_word2vec_distances(
        ...
        if "Vector" in w2v_df.columns:
            ...
            if keywords_var:
                ...
                keyword_sim_outputFilename = IO_files_util.generate_output_file_name(
                    inputFilename, inputDir, outputDir, ".csv", f"Word2Vec_{len(keywords_list)}_Keywords_Cos_Similarity"
                )
                keyword_df.to_csv(keyword_sim_outputFilename, encoding="utf-8", index=False)
                filesToOpen.append(keyword_sim_outputFilename)
            else:
                logger.error(
                    'You have selected a vector csv file in input. This I/O option allows you to compute cosine similarity distances between selected keywords. But the "Keywords" widget is empty.\n\nPlease, enter comma-separated words in the "Keywords" widget and try again.'
                )
            return filesToOpen
        else:
            logger.error(
                "The selected csv file does NOT contain Vector column.\n\nPlease, select a different csv file and try again."
            )
    ...
    if n_dim_Euclidean:
        logger.info(
            f"Started computing word distances between top {top_words_var} words at {time.asctime(time.localtime(time.time()))}"
        )

        dist_rows = []
        logger.info(
            f"Started computing n-dimensional Euclidean distance between top {top_words_var} words at {time.asctime(time.localtime(time.time()))}"
        )
    ...
    if compute_cosine_similarity:
        cos_rows = []
        logger.info(
            f"Started computing cosine similarity between top {top_words_var} words at {time.asctime(time.localtime(time.time()))}"
        )
        ...
        if keywords_var:
            keywords_list = [x.strip() for x in keywords_var.split(",")]
            logger.info(
                f"Started computing cosine similarity between words for {len(keywords_list)} selected keywords at {time.asctime(time.localtime(time.time()))}"
            )
            kw_rows = []
            for a, b in itertools.combinations(keywords_list, 2):
                try:
