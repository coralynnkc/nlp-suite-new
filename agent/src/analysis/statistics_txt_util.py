...
def compute_character_word_ngrams(
    ...
    if inputFilename == "" and inputDir == "":
        logger.info('Input error No input file or input directory have been specified.\n\nThe function will exit.\n\nPlease, enter the required input options and try again.')
        return
...
def compute_sentence_text_readability(
    ...
    if len(inputDir) != 0:
        logger.info('Warning %s', "The output filenames generated by Textstat readability contain the name of the directory processed in input, rather than the name of any individual file in the directory.\n\nBoth txt & csv files include all " + str(nFile) + " files in the input directory processed by Textstat.")
    return filesToOpen
...
def sentence_structure_tree(inputFilename, outputDir, num_sentences):
    if inputFilename == "":
        ...
        if maxNum >= 10:
            logger.info('Warning The number of sentences entered is quite large. The tree graph algorithm will produce a png file for every sentence')
